 ...
 * added yyAvax state variable and setyyAvax() function
 * adjusted pricicing feed to get rate from
 * YY StakedAvax contract Mainnet : 0xf7d9281e8e363584973f946201b82ba72c965d27
 * Can only handle WAVAX and yyAvax
 */
contract PoolYYAvax is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, Core, IPool {
    ...
    /// @notice An event emitted when yyAvax address changes
    event yyAvaxUpdated(address previousyyAvax, address newyyAvax);
    ...
    function withdrawFromOtherAsset(
        ...
        // yyAvax -> AVAX
        if (initialToken == yyAvax && wantedToken == weth) {
            liquidityInInitialAssetDP = liquidityInInitialAssetDP.wdiv(yyAvaxRate);
            // AVAX -> yyAvax
        } else if (initialToken == weth && wantedToken == yyAvax) {
     ...
     * @param toAsset The asset wanted by user
     * @param fromAmount The amount to quote
     * @dev assumes only yyAvax and AVAX assets can be swaped for this pool.
    ...
    function _quoteIdealToAmount(
        ...
        // yyAvax -> AVAX => apply rate
        if (fromToken == yyAvax && toToken == weth) {
            idealToAmount = fromAmount.wmul(yyAvaxRate);

            // AVAX -> yyAvax => divide rate
        } else if (fromToken == weth && toToken == yyAvax) {
            idealToAmount = fromAmount.wdiv(yyAvaxRate);
        } else {
            // we assume only these two tokens can be added to the pool
        ...
        returns (uint256 maxInitialAssetAmount)
    {
        ...
        if (wantedAssetCov > ETH_UNIT) {
            ...
            // yyAvax -> AVAX
            if (initialToken == yyAvax && wantedToken == weth) {
                maxInitialAssetAmount = maxInitialAssetAmount.wdiv(yyAvaxRate);
                // AVAX -> yyAvax
            } else if (initialToken == weth && wantedToken == yyAvax) {
