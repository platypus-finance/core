...
describe('AvaxPool Swap Avax', function () {
  ...
  beforeEach(async function () {
    ...
    // avax pool
    const poolSetup = await setupAAvaxCPool(owner)
    this.pool = poolSetup.pool
    this.WETH = poolSetup.WETH

    await setupAAvaxCPriceFeed(this.pool)
  })
  ...
  describe('Asset WAVAX (weth)', function () {
    ...
    describe('swapFromETH', function () {
      it('works (AVAX -> aAvaxc)', async function () {
        const beforeFromBalance = await ethers.provider.getBalance(users[0].address)
        const beforeToBalance = await this.aAvaxc.balanceOf(users[1].address)
        const receipt = await this.pool
          .connect(users[0])
          .swapFromETH(this.aAvaxc.address, parseEther('0'), users[1].address, this.fiveSecondsSince, {
            value: parseEther('1'),
            gasPrice: 69666333,
          })
        ...
        const afterToBalance = await this.aAvaxc.balanceOf(users[1].address)
      ...
      it('reverts if passed deadline', async function () {
        await expect(
          this.pool
            .connect(users[0])
            .swapFromETH(this.aAvaxc.address, parseEther('0'), users[0].address, this.fiveSecondsAgo, {
              value: parseEther('1'),
            }),
          'Platypus: EXPIRED'
        ).to.be.reverted
      ...
      it('reverts if amount to receive is less than expected', async function () {
        await expect(
          this.pool
            .connect(users[0])
            .swapFromETH(this.aAvaxc.address, parseEther('10000'), users[0].address, this.fiveSecondsSince, {
              value: parseEther('1'),
            }),
      ...
      it('reverts if pool paused', async function () {
        ...
        await expect(
          this.pool
            .connect(users[0])
            .swapFromETH(this.aAvaxc.address, parseEther('0'), users[0].address, this.fiveSecondsSince, {
              value: parseEther('1'),
            }),
          'Pausable: paused'
        ).to.be.reverted
    ...
    describe('swapToETH', function () {
      it('works', async function () {
        const beforeFromBalance = await this.aAvaxc.balanceOf(users[0].address)
        ...
        const receipt = await this.pool
          .connect(users[0])
          .swapToETH(this.aAvaxc.address, parseEther('5'), parseEther('0'), users[1].address, this.fiveSecondsSince)

        const afterFromBalance = await this.aAvaxc.balanceOf(users[0].address)
      ...
      it('reverts if passed deadline', async function () {
        await expect(
          this.pool
            .connect(users[0])
            .swapToETH(this.aAvaxc.address, parseEther('500'), parseEther('0'), users[0].address, this.fiveSecondsAgo),
          'Platypus: EXPIRED'
        ).to.be.reverted
      })
