...
describe('AvaxPool Deposit Avax', function () {
  ...
  describe('Asset aAvaxc', function () {
    beforeEach(async function () {
      const aggregateName = 'Liquid staking AVAX Aggregate'
      const aggregateAccount = await setupAggregateAccount(owner, aggregateName, true)
      await aggregateAccount.deployTransaction.wait()
      ...
      const tokenSetSAvax = await createAndInitializeToken('aAvaxc', 18, owner, this.pool, aggregateAccount)

      this.aAvaxc = tokenSetSAvax.token
    ...
    describe('deposit', function () {
      beforeEach(async function () {
        ...
        await this.aAvaxc.connect(owner).transfer(users[0].address, parseEther('100000')) // 100 k
        // Approve max allowance from users[0] to pool
        await this.aAvaxc.connect(users[0]).approve(this.pool.address, ethers.constants.MaxUint256)
      })

      it('works (first LP)', async function () {
        // Get aAvaxc balance of user[0]
        const beforeBalance = await this.aAvaxc.balanceOf(users[0].address)
        // Deposit from user[0] to pool 100 aAvaxc
        const receipt = await this.pool
          .connect(users[0])
        ...
        await expect(receipt)
          .to.emit(this.pool, 'Deposit')
          .withArgs(users[0].address, this.aAvaxc.address, parseEther('100'), parseEther('100'), users[0].address)
      })
      ...
      it('works (second LP)', async function () {
        ...
        await this.aAvaxc.connect(owner).transfer(users[1].address, parseEther('100000'))
        await this.aAvaxc.connect(users[1]).approve(this.pool.address, ethers.constants.MaxUint256)
        // Deposit from user[1] to pool 100 aAvaxc
        await this.pool
          .connect(users[1])
          .deposit(this.aAvaxc.address, parseEther('100'), users[1].address, this.fiveSecondsSince)
      ...
      it('Respects asset max supply', async function () {
        ...
        await expect(
          this.pool
            .connect(users[0])
            .deposit(this.aAvaxc.address, parseEther('101'), users[2].address, this.fiveSecondsSince)
        ).to.be.revertedWith('MAX_SUPPLY_REACHED')
      ...
      it('applies deposit fee when cov > 1 to prevent deposit arbitrage', async function () {
        ...
        // Next, adjust coverage ratio to = 2 > 1
        await this.asset.connect(owner).setPool(owner.address)
        await this.aAvaxc.connect(owner).transfer(this.asset.address, parseEther('100')) // transfer aAvaxc to Asset to back
        await this.asset.connect(owner).addCash(parseEther('100'))
        ...
        await this.pool
          .connect(users[0])
          .deposit(this.aAvaxc.address, parseEther('100'), users[0].address, this.fiveSecondsSince)
        const afterBalance2 = await this.aAvaxc.balanceOf(users[0].address)
        ...
        // Now, try to withdraw 100 and see if we can perform arbitrage : if amount withdrawn is > 100
        const beforeBalance3 = await this.aAvaxc.balanceOf(users[0].address)

        const [quotedWithdrawal] = await this.pool.quotePotentialWithdraw(this.aAvaxc.address, parseEther('100'))
        ...
        const receipt3 = await this.pool
          .connect(users[0])
          .withdraw(this.aAvaxc.address, parseEther('100'), parseEther('0'), users[0].address, this.fiveSecondsSince)
        const afterBalance3 = await this.aAvaxc.balanceOf(users[0].address)
      ...
      it('reverts if passed deadline', async function () {
        await expect(
          this.pool
            .connect(users[0])
            .deposit(this.aAvaxc.address, parseEther('100'), users[0].address, this.fiveSecondsAgo),
          'EXPIRED'
        ).to.be.reverted
      })
      ...
      it('reverts if liquidity to mint is too small', async function () {
        await expect(
          this.pool
            .connect(users[0])
            .deposit(this.aAvaxc.address, parseEther('0'), users[0].address, this.fiveSecondsSince),
