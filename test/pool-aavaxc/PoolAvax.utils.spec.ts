...
import { setupAAvaxCPool, setupAAvaxCPriceFeed, usdc } from '../helpers/helper'
...
describe('AvaxPool Utils', function () {
  ...
  describe('Get and set slippage params, haircut and retention ratio', function () {
    ...
    it('sets and gets sAvax', async function () {
      // gets sAvax
      assert.exists(await this.pool.connect(owner).aAvaxc(), 'Param OK')

      // Can set WETHForwarder
      await this.pool.connect(owner).setAAvaxc('0xd00ae08403B9bbb9124bB305C09058E32C39A48c')
      expect(await this.pool.connect(owner).aAvaxc()).to.be.equal('0xd00ae08403B9bbb9124bB305C09058E32C39A48c')
    })
