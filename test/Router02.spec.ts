...
describe('Router02', function () {
  ...
  beforeEach(async function () {
    ...
    Router = await ethers.getContractFactory('PlatypusRouter02')
  })
  ...
  describe('Router Swap', function () {
    it('Should swap using only 1 pool (DAI -> USDC)', async function () {
      ...
      const [quotedAmount, quotedHaircut] = await this.router
        .connect(users[0])
        .quotePotentialSwaps([this.DAI.address, this.USDC.address], [this.pool1.address], parseEther('100'))

      // get actual haircut from swap using call static
      const [, actualHaircut] = await this.router.connect(users[0]).callStatic.swapTokensForTokens(
      ...
      const poolHaircut = await this.pool1.getHaircutRate()

      expect(quotedHaircut).to.be.equal(poolHaircut.mul(100))
      expect(actualHaircut).to.be.equal(poolHaircut.mul(100))
    ...
    it('Should swap using 2 pools (DAI -> USDC -> TSD)', async function () {
      ...
      const [quoteAmount, quotedHaircut] = await this.router
        .connect(users[0])
        .quotePotentialSwaps(
      ...
      expect(quotedHaircut).to.be.equal(parseEther('0.079983103632773208'))
      expect(actualHaircut).to.be.equal(parseEther('0.079983103632773208'))
